 ...
 > 装箱排箱计算器 — 从外箱 / 内箱 / 销售盒尺寸出发，自动给出多方案装箱排布、成本对比、EPE 珍珠棉匹配与包装图纸输出。
 ...
 - **Seeed 物流纸箱库**：内置常见通用物流纸箱尺寸、材质、编码与已有珍珠棉备注，直接选型无需翻表。
 - **EPE 珍珠棉匹配**：内置珍珠棉 SKU 尺寸库与最终面位匹配表，自动给出每面珍珠棉编码、数量与方向校验。
 - **方向与间隙控制**：支持保持直立 / 允许翻转，支持满填充优先与严格间隙两种模式。
 - **人体工学提醒**：当外箱尺寸或重量超出搬运建议时给出警告，而非直接过滤。
 - **图纸输出**：
   - `SVG` 预览图 — 快速查看装箱排布
   - `DXF` 工程图 — ZWCAD / CAD 可直接打开
   - `PDF` 出图风格报告 — 手机端可导出，含装箱数、内箱数、珍珠棉分配与编码
   - `STL` 几何（可选）— 可导入 Creo 做三维参考
 ...
 > 以下为应用界面预览，实际效果以[在线版本](https://sery-921.github.io/Packing-Calculator/)为准。

 | 输入尺寸 | 方案对比 | 图纸预览 |
 |:---:|:---:|:---:|
 | ![输入界面](https://sery-921.github.io/Packing-Calculator/assets/preview-input.png) | ![方案对比](https://sery-921.github.io/Packing-Calculator/assets/preview-plans.png) | ![图纸预览](https://sery-921.github.io/Packing-Calculator/assets/preview-drawing.png) |

 > 💡 截图为占位，可后续替换为真实截图。建议放在仓库 `assets/` 目录下。
 ...
 2. **选择纸箱**：从内置 Seeed 物流纸箱库选择，或手动填入自定义纸箱尺寸。
 3. **查看方案**：计算结果会列出多个可行方案，包含每箱装箱数、利用率、所需纸箱数与成本。
 4. **珍珠棉匹配**：自动给出各面珍珠棉编码、数量与方向，供包装作业参考。
 5. **导出图纸**：按需导出 SVG 预览、DXF 工程图或 PDF 报告。
 ...
 - STL 几何仅作三维参考，无参数特征树，导入 Creo 后需重新建模。
